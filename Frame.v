...
fn (app App) lobby(transparence u8) {
	...
	app.text_rect_render(int(app.win_width / 2), int(app.win_height / 2) + 40, 'Press ' +
		key_code_name[app.list_action_key_code[Actions.start]], transparence)
	mut y := 0
	if app.player_nb <= 2 {
	...
	for p_nb in 0 .. app.player_nb {
		...
		str := key_code_name[app.list_action_key_code[6 + 3 * p_nb]] + ' ' +
			key_code_name[app.list_action_key_code[7 + 3 * p_nb]] + ' ' +
			key_code_name[app.list_action_key_code[8 + 3 * p_nb]]
